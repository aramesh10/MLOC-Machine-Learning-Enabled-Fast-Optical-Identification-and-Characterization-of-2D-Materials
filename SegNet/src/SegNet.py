...
class Train():
    ...
    @staticmethod
    def Train(trainloader, path=None, view_results=False): #epochs is target epoch, path is provided to load saved checkpoint

        model = SegNet()
        optimizer = torch.optim.SGD(model.parameters(), lr=LEARNING_RATE, momentum=MOMENTUM)
        loss_fn = nn.CrossEntropyLoss()
        run_epoch = EPOCHS
        model_file = 'segnet_weights.pth.tar'

        avg_loss = []

        if path == None:
            epoch = 0
            path = os.path.join(os.getcwd(), f'./model/{model_file}')
        ...
        for i in range(1, run_epoch + 1):
            ...
            for j, data in enumerate(trainloader, 1):
                ...
                # Save and plot training error and loss

                sum_loss += loss.item()

                print('Loss at {} mini-batch: {}'.format(j, loss.item()/trainloader.batch_size))

            avg_loss.append(sum_loss/j*trainloader.batch_size)
