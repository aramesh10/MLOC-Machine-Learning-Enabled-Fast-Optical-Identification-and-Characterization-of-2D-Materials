...
class GrapheneDataloader:
    ...
    def __getitem__(self, i): 
        ...
        for j in range(start, stop):
            data.append([self.dataset[j][0], self.dataset[j][1]])
            # print("DATASET:")
            # print([self.dataset[j][0], self.dataset[j][1]])
        # transpose list of lists
        # batch = [np.stack([samples, mask], axis=0) for samples, mask in zip(*data)]
...
if __name__ == '__main__':
    ...
    train_graphene_dataset = Dataset(x_train_dir, y_train_dir, classes=['SUBSTRATE', 'MONO-LAYER', 'MULTI-LAYER'])
    val_graphene_dataset = Dataset(x_val_dir, y_val_dir, classes=['SUBSTRATE', 'MONO-LAYER', 'MULTI-LAYER'])
    test_graphene_dataset = Dataset(x_test_dir, y_test_dir, classes=['SUBSTRATE', 'MONO-LAYER', 'MULTI-LAYER'])
