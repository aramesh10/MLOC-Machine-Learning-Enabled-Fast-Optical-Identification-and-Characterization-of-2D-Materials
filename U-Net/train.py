...
from dataset import GrapheneDataset, post_process_mask_prediction, calc_metrics

RUN_NAME = "dice_iou_none_normalized_preprocess"
EPOCHS = 20
BATCH_SIZE = 4
...
if __name__ == '__main__':
    ...
    train_graphene_dataset = GrapheneDataset(x_train_dir, y_train_dir, preprocess_bool=False, img_format='spectral', normalize=True)
    val_graphene_dataset   = GrapheneDataset(x_val_dir, y_val_dir, preprocess_bool=False, img_format='spectral', normalize=True)
    test_graphene_dataset  = GrapheneDataset(x_test_dir, y_test_dir, preprocess_bool=False, img_format='spectral', normalize=True)
    ...
    for image, label in tqdm(test_graphene_dataset):
        x_tensor = image.to(DEVICE).unsqueeze(0)
        prediction = post_process_mask_prediction(model.predict(x_tensor))
        
        overall_accuracy, label_acc, confusion_mat, precision, recall, f1_score = calc_metrics(prediction, label)
    ...
    confusion_matrix_norm = confusion_matrix / confusion_matrix.astype(np.float32).sum(axis=0)

    # Print metric
    print("*"*50)
    print("Precision:", np.nanmean(precision_arr))
    print("Recall:", np.nanmean(recall_arr))
    print("F1 Score:", np.nanmean(f1_arr))
    print()
    ...
    print("*"*50)
    print(f"Wrote to ./U-Net/runs/{RUN_NAME}/metrics.txt")

    # Write results to file
    with open(f"./U-Net/runs/{RUN_NAME}/metrics.txt", "w+") as metric_file:
        ...
        metric_file.write(f"\n")
        metric_file.write(f"Normalized Confusion Matrix\n")
        metric_file.write(f"{confusion_matrix_norm}\n")
        metric_file.write(f"\n")
